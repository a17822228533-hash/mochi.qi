...
@asynccontextmanager
async def make_lifespan(token: str):
    """创建lifespan context manager，yield token供handler使用"""
    yield token

def make_server(token):
    """创建Server实例，传入lifespan"""
    lifespan_cm = make_lifespan(token)
    return Server('mochi-mcp', lifespan=lifespan_cm, on_list_tools=handle_list_tools, on_call_tool=handle_call_tool)
