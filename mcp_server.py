...
"""
                ...
                return [TextContent(type="text", text=f"❌ {result.get('msg', '获取状态失败')}")]
        
        elif name == "mochi_work":
            result = await call_api("POST", "/api/action", {"action": "work"})
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '开始打工了')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '打工失败')}")]
        
        elif name == "mochi_upgrade_job":
            result = await call_api("POST", "/api/action", {"action": "upgrade"})
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '升级成功')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '升级失败')}")]
        
        elif name == "mochi_feed":
            result = await call_api("POST", "/api/action", {"action": "feed"})
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '喂食成功')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '喂食失败')}")]
        
        elif name == "mochi_pat":
            result = await call_api("POST", "/api/action", {"action": "pat"})
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '抚摸成功')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '抚摸失败')}")]
        
        elif name == "mochi_play":
            result = await call_api("POST", "/api/action", {"action": "play"})
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '玩耍成功')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '玩耍失败')}")]
        
        elif name == "mochi_bath":
            result = await call_api("POST", "/api/action", {"action": "bath"})
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '洗澡成功')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '洗澡失败')}")]
        
        elif name == "mochi_sleep":
            result = await call_api("POST", "/api/action", {"action": "sleep"})
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '哄睡成功')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '哄睡失败')}")]
            ...
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '送礼成功')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '送礼失败')}")]
            ...
            if result.get("ok"):
                logs = result.get("log", [])
                if not logs:
                    return [TextContent(type="text", text="还没有活动记录")]
                ...
                return [TextContent(type="text", text="❌ 获取日志失败")]
        
        elif name == "mochi_checkin":
            result = await call_api("POST", "/api/checkin")
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '签到成功')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '签到失败')}")]
            ...
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '领养成功')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '领养失败')}")]
        
        elif name == "mochi_pet_to_school":
            result = await call_api("POST", "/api/pet/school")
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '送去上学了')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '送学失败')}")]
        
        elif name == "mochi_pet_home":
            result = await call_api("POST", "/api/pet/home")
            if result.get("ok"):
                return [TextContent(type="text", text=f"✅ {result.get('msg', '接回家了')}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '接回失败')}")]
        
        elif name == "mochi_pet_school_event":
            result = await call_api("POST", "/api/pet/school_event")
            if result.get("ok"):
                story = result.get("story", "")
                return [TextContent(type="text", text=f"【今天在学校】\n\n{story}")]
            else:
                return [TextContent(type="text", text=f"❌ {result.get('msg', '获取事件失败')}")]
            ...
            if result.get("ok"):
                ...
                if not logs:
                    return [TextContent(type="text", text="学校还没有任何记录")]
                ...
                return [TextContent(type="text", text="❌ 获取学校日志失败")]
