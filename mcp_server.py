...
async def websocket_handler(request):
    ...
    print("WebSocket连接建立", file=sys.stderr)
    ...
    try:
        async for msg in ws:
            if msg.type == web.WSMsgType.TEXT:
                ...
                print(f"WebSocket错误: {ws.exception()}", file=sys.stderr)
    
    finally:
        print("WebSocket连接关闭", file=sys.stderr)
...
def main():
    ...
    print(f"Mochi MCP WebSocket服务启动在端口 {port}", file=sys.stderr)
