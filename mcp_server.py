...
async def handle_call_tool(ctx, params):
    token = CURRENT_TOKEN  # 从全局变量读取token
    name = params.name
...
def make_server(token):
    """创建Server实例，不再使用lifespan传token"""
    return Server('mochi-mcp', on_list_tools=handle_list_tools, on_call_tool=handle_call_tool)
