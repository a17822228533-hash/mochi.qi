...
def make_server(token):
    ...
    async def lifespan(server):
        """Lifespan context manager that yields the token for handlers to use"""
        yield token
    
    return Server('mochi-mcp', lifespan=lifespan, on_list_tools=handle_list_tools, on_call_tool=handle_call_tool)
